...
Enrich a specific scope (user story) that is underspecified, ambiguous, or incomplete. Does not change scope status — only deepens the definition so it is ready to execute.
...
`$ARGUMENTS` — format: `NNN-slug scope-NN` (e.g. `001-user-auth-api scope-03`)
...
4. Diagnose the scope for issues. Check each of the following and list which apply:
   ...
   - **Undefined acceptance conditions** — no clear pass/fail boundary for the scope.
...
6. Apply the enrichments to the scope file:
...
7. Execute `[CHECK-AGNOSTIC-BOUNDARY]` — verify the enriched scope is consistent with `docs/` contracts.
...
> Does NOT change scope status (TODO / IN PROGRESS / DONE). Does NOT execute any tasks.
