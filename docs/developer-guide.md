...
Architecture, internals, and contribution reference for the `claude-planning-with-ai` plugin.
...
│   └── verify-plugin.sh     ← local structural validation for plugin maintainers
...
The `name` field becomes the plugin identifier. The `version` follows semver. Major versions are appropriate when installed templates, command contracts, or generated planning file shapes change in a way users should consciously adopt.
...
6. Run `bash scripts/verify-plugin.sh` before committing.
...
| Execution | `02-EXECUTION-WORKFLOWS/` | ATOMIZE-STORY, GENERATE-DOCUMENT, REVIEW-COHERENCE, EXPAND-ELEMENT, INTEGRATE-MILESTONE |
...
| Project guidance | `06-PROJECT-GUIDANCE/` | Adaptation notes for documentation, research, operations, and general projects |
...
There is no compiler, schema engine, or alternative authoring format — the markdown files in `.planning/` are the only representation of a planning. The `/plan-validate` skill provides structural integrity checking: it reads the planning markdown directly and verifies file locations per lifecycle state, story table ↔ story file consistency, workflow IDs against the `WORKFLOWS/` catalog, dependency references, and atomized task folders (index ↔ file consistency, required sections, dependency cycles). For deep auditing of atomic tasks against the `[CHECK-ATOMICITY]` requirements, `/plan-task-validate` covers the task level. Both are read-only and report findings without modifying files.

For plugin-maintainer validation, run:

```bash
bash scripts/verify-plugin.sh
```

The script checks required files, skill frontmatter, `docs/commands.yml` coverage, referenced skill sources, workflow groups, template risk/external issue markers, and public command references.
...
- Update `.page/locales/commandsPage.en.ts` and `.page/locales/commandsPage.es.ts` if the command is user-facing
- Keep `CLAUDE.md` in sync if the architectural constraint changes
- Check for obsolete command names or concepts (`/plan-scope`, `scope-NN`, `doc-scope`) whenever a rename affects the public workflow.
...
2. Running `bash scripts/verify-plugin.sh`
3. Running the command in a Claude Code session against a real project
4. Running `npm run build` in `.page/` before any landing page change
